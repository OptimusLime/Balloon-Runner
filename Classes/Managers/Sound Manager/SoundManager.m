...
- (void)loadBackgroundMusicWithFile:(NSString*)aMusicFile {
    ...
    // Check to make sure that a sound with the same key does not already exist
    NSString *path = [musicLibrary objectForKey:aMusicFile];
    
    // If the key is found log it and finish
    if(path != nil) {
        NSLog(@"WARNING - SoundManager: Music with the key '%@' already exists.", aMusicFile);
        return;
    }
	...
	[musicLibrary setObject:path forKey:aMusicFile];
    NSLog(@"INFO - SoundManager: Loaded background music with key '%@'", aMusicFile);
}
