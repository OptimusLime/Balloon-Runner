...
static void addShape(cpSpace *space, void *obj, void *data)
{
	...
	{
		//only add if the body isn't already added to the space
...
-(void) removeAndMaybeFreeShape:(cpShape*)shape freeShape:(BOOL)freeShape
{
	...
	if (freeShape)
	{
		[_shapeArray removeObject:[NSValue valueWithPointer:shape]];
		cpShapeFree(shape);	
	}
}
...
-(void) addShape:(cpShape*)shape
{
	...
	[_shapeArray addObject: [NSValue valueWithPointer: shape]];
}
-(void) clearOutRemainingShapes
{
	NSValue* sVal;
	cpShape* rem;
	for(sVal in _shapeArray)
	{
		rem = (cpShape*)[sVal pointerValue];
	}
